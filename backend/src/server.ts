...
// The EVM server never had this — every async route handler that throws
// before reaching P0.4's gate hangs the request forever (an unhandled
// promise rejection Express 4 does not forward to error middleware on its
// own), rather than a clean 500. server.fabric.ts already imports this;
// pulling it in here too closes the same gap on the EVM entrypoint.
