...
///
/// P1.2: the cookie carries the raw session token, but only its SHA-256
/// hash is ever looked up against the `sessions` table — the table itself
/// never stores a directly-usable token (see utils/session-token.ts).
